...
export default function InvitePage({ params }: { params: { slug: string } }) {
  const slug = params.slug   // ✅ Next.js 14: langsung akses, TIDAK pakai use(params)
  ...
  useEffect(() => {
    async function validate() {
      ...
      if (error || !linkData) { setStatus('invalid'); return }
      ...
      supabase.rpc('increment_link_usage', { link_slug: slug }).then(() => {})
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (error) { setErr('Gagal mengirim, coba lagi.'); setStatus('valid') }
  ...
  if (status === 'loading') return (
    <Screen>
      <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:14 }}>
        <div style={{ width:38, height:38, border:'3px solid #1F1F23', borderTopColor:'#FF2D20', borderRadius:'50%', animation:'spin3d 1s linear infinite' }} />
        <p style={{ color:'#3A3A42', fontSize:'.8rem', fontFamily:'JetBrains Mono,monospace' }}>Memvalidasi link…</p>
      </div>
    </Screen>
  )
  ...
  if (status === 'invalid') return (
    <Screen>
      <StatusCard icon="🔗" iconBg="rgba(255,45,32,.1)" iconBorder="rgba(255,45,32,.2)"
        title="LINK TIDAK VALID" titleColor="#FF2D20"
  ...
  if (status === 'expired') return (
    <Screen>
      <StatusCard icon="⏰" iconBg="rgba(245,158,11,.1)" iconBorder="rgba(245,158,11,.2)"
        title="LINK KEDALUWARSA" titleColor="#F59E0B"
  ...
  if (status === 'success') return (
    <Screen>
      <div style={{ textAlign:'center', maxWidth:400 }}>
        ...
        <h2 className="font-display" style={{ fontSize:'2.8rem', color:'#F0F0F0', marginBottom:10 }}>TERIMA KASIH!</h2>
        <p style={{ color:'#6B6B78', fontSize:'.9rem', lineHeight:1.75, marginBottom:6 }}>
          Testimonimu sudah masuk dan sedang menunggu persetujuan.
        </p>
        <p style={{ color:'#3A3A42', fontSize:'.78rem', fontFamily:'JetBrains Mono,monospace', marginBottom:32 }}>Biasanya tampil dalam 1×24 jam 🙏</p>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#070708' }}>
      ...
      {/* Topbar */}
      <div style={{ borderBottom:'1px solid #1F1F23', padding:'0 20px', height:56, display:'flex', alignItems:'center', justifyContent:'space-between', position:'sticky', top:0, background:'rgba(7,7,8,.92)', backdropFilter:'blur(16px)', zIndex:50 }}>
        <Link href="/" style={{ textDecoration:'none' }}>
          <span className="font-display" style={{ fontSize:'1.3rem', color:'#F0F0F0' }}>KOGRAPHH<span style={{ color:'#FF2D20' }}>.</span></span>
        </Link>
      ...
      <div style={{ maxWidth:520, margin:'0 auto', padding:'48px 20px 80px' }}>
        ...
        <div style={{ marginBottom:36 }}>
          ...
          <h1 className="font-display" style={{ fontSize:'clamp(2.4rem,6vw,3.6rem)', color:'#F0F0F0', lineHeight:1, marginBottom:10 }}>BERI TESTIMONI</h1>
          {link?.product_name ? (
            <div style={{ display:'inline-flex', alignItems:'center', gap:8, background:'#0E0E10', border:'1px solid #1F1F23', borderRadius:10, padding:'8px 14px', marginTop:10 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#6B6B78" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 3l-4 4-4-4"/></svg>
            ...
            <p style={{ color:'#6B6B78', fontSize:'.85rem', lineHeight:1.7, marginTop:8 }}>Pengalamanmu sangat berarti bagi kami dan pembeli lainnya.</p>
          )}
        </div>
        ...
        <form onSubmit={submit} style={{ display:'flex', flexDirection:'column', gap:22 }}>
          ...
          {/* Name */}
          <Field label="NAMA *">
            <input className="inp" value={name} onChange={e => setName(e.target.value)} placeholder="Nama kamu (boleh pakai inisial)" required />
          </Field>
          ...
          <Field label="PRODUK YANG DIBELI">
            {link?.product_id ? (
              <div style={{ background:'#0E0E10', border:'1px solid #1F1F23', borderRadius:12, padding:'12px 16px', display:'flex', alignItems:'center', gap:10 }}>
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#22C55E" strokeWidth="2"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 3l-4 4-4-4"/></svg>
          ...
          <Field label="CERITAKAN PENGALAMANMU *">
            ...
            <div style={{ display:'flex', justifyContent:'space-between', marginTop:5 }}>
              ...
              {message.length >= 10 && <span style={{ color:'#22C55E', fontSize:'.68rem', fontFamily:'JetBrains Mono,monospace' }}>✓ ok</span>}
            </div>
          </Field>
          ...
          <button type="submit" disabled={status === 'submitting'} style={{
            background:'linear-gradient(135deg,#FF2D20 0%,#9333EA 100%)', color:'#fff', border:'none',
            borderRadius:14, padding:'15px', fontSize:'.92rem', fontWeight:700,
...
function StatusCard({ icon, iconBg, iconBorder, title, titleColor, desc }: {
  ...
  return (
    <div style={{ textAlign:'center', maxWidth:380 }}>
      ...
      <h2 className="font-display" style={{ fontSize:'2.2rem', color:titleColor, marginBottom:10 }}>{title}</h2>
